...
<template>
    <div>
        <Menubar
            :model="adminHeader ? adminItems : userItems"
            class="bg-green-50 border-b border-b-emerald-800 rounded-none py-0"
        >
            <template #start>
                <h1 class="font-bold text-2xl text-emerald-800 mx-4">
                    Calcul'UT
                </h1>
            </template>
            <template #item="{ item, props, hasSubmenu }">
                <a :href="item.url" :target="item.target" v-bind="props.action" class="mx-2">
